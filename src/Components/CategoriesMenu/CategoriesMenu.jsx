...
export default function CategoriesMenu({ApiBaseUrl , useQuery , axios}) {
  ...
  return <>
      <div className="categoriesMenu bg-light rounded p-3 mb-4">
        ...
        <div>
          {data?.data?.data?.data.map((category) => <h6 className='cursor-pointer' key={category?._id}>{category?.name}</h6>)}
        </div>
    </div>
